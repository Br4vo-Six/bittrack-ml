...
from src.models.common.tx import CommonTx, AbstractCommonTx
from src.utils.datetime_util import timestamp_to_datetime
...
class Tx(AbstractCommonTx):
    ...
    def block_height(self):
        """Height of the block that contains this transaction. If this is an unconfirmed transaction, it will equal
        -1."""
    ...
    @property
    def preference(self):
        """The likelihood that this transaction will make it to the next block; reflects the preference level miners
        have to include this transaction. Can be high, medium or low."""
    ...
    @property
    def lock_time(self):
        """Time when transaction can be valid. Can be interpreted in two ways: if less than 500 million, refers to
        block height. If more, refers to Unix epoch time."""
    ...
    def confirmations(self):
        """Number of subsequent blocks, including the block the transaction is in. Unconfirmed transactions have 0
        confirmations."""
    ...
    @property
    def opt_in_rbf(self):
        """Returns true if this transaction has opted in to Replace-By-Fee (RBF), either true or not present. You can
        read more about Opt-In RBF here."""
        return self.__opt_in_rbf

    @property
    def confidence(self):
        """The percentage chance this transaction will not be double-spent against, if unconfirmed. For more
        information, check the section on Confidence Factor."""
    ...
    def change_address(self):
        """Address BlockCypher will use to send back your change, if you constructed this transaction. If not set,
    ...
    def data_protocol(self):
        """Returned if this transaction contains an OP_RETURN associated with a known data protocol. Data protocols
        currently detected: blockchainid ; openassets ; factom ; colu ; coinspark ; omni"""
    ...
    def next_inputs(self):
        """If there are more transaction inptus that couldn't fit into the TXInput array, this is the BlockCypher URL
        to query the next set of TXInputs (within a TX object)."""
        return self.__next_inputs

    @property
    def next_outputs(self):
        """If there are more transaction outputs that couldn't fit into the TXOutput array, this is the BlockCypher
        URL to query the next set of TXOutputs(within a TX object)."""
